defmodule EducatiumWeb.HomeLive.Components.MultipleResourcesForm do
    ...
    <div>
      ...
      >
        ...
        <div class={@status && "hidden"}>
          <.label for="dirs"><%= gettext("Upload a directory to join your resource") %></.label>
          <.live_file_input upload={@uploads.dirs} class="hidden" />
          <input
  ...
  @impl true
  def handle_event("validate", %{"_target" => ["dirs"]}, socket) do
    IO.puts("Validating directories...")
  ...
  defp handle_progress(:dirs, entry, socket) do
    if entry.done? do
      File.mkdir_p!(@uploads_dir)

      [{dest, paths}] =
        consume_uploaded_entries(socket, :dirs, fn %{path: path}, _entry ->
